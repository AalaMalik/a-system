...
class WebSocketService {
  ...
  async connect(userId) {
    ...
    let wsUrl;
    try {
      // Get WebSocket URL from utility (returns null if HTTPS/ngrok)
      const backendBaseUrl = getWebSocketUrl();
      
      // If WebSocket is disabled (accessed via HTTPS), skip connection
      if (!backendBaseUrl) {
        console.warn('WebSocket disabled: Page accessed via HTTPS. WebSocket requires wss:// or HTTP access.');
        return; // Skip WebSocket connection
      }
      
      wsUrl = `${backendBaseUrl}/ws/attendance/notifications/${userId}/`;
